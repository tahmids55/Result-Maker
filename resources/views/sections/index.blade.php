...
<div class="py-4 space-y-4">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
        <form method="GET" action="{{ route('sections.index') }}" class="flex flex-wrap md:flex-nowrap gap-3">
            <div class="flex-1">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('sections.index') }}" class="border border-gray-300 text-gray-600 hover:bg-gray-50 text-sm px-3 py-2 rounded-lg transition-colors flex items-center">
                    ✕
                </a>
            </div>
        </form>
    ...
    <div class="flex justify-between items-center">
        <p class="text-sm text-gray-500">{{ $sections->total() }} sections found</p>
        <div class="flex gap-2">
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" x-cloak @click.away="open=false"
                    ...
                    <form method="POST" action="{{ route('sections.bulk-import') }}" enctype="multipart/form-data" class="space-y-2">
                        @csrf
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-gray-700 font-medium">{{ $section->schoolClass->name }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $section->name }}</td>
                        <td class="px-4 py-3 text-center text-gray-500">{{ $section->students_count }}</td>
