...
<div class="py-4 max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <h2 class="text-base font-semibold text-gray-800 mb-5">Add Subject</h2>

        <form method="POST" action="<?php echo e(route('subjects.store')); ?>" x-data="subjectForm()" class="space-y-5">
            <?php echo csrf_field(); ?>

            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Subject Name *</label>
                    <input type="text" name="name" value="<?php echo e(old('name')); ?>" required placeholder="e.g. Mathematics"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Subject Code</label>
                    <input type="text" name="code" value="<?php echo e(old('code')); ?>" placeholder="e.g. MAT"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                ...
                <div class="col-span-2">
                    <label class="flex items-center gap-2 text-sm font-medium text-gray-700 cursor-pointer">
                        <input type="checkbox" name="is_optional" value="1" <?php echo e(old('is_optional') ? 'checked' : ''); ?> class="rounded">
                        Optional Subject
                    </label>
                </div>
            </div>
            ...
            <div x-show="!hasSubSubjects" class="mt-4 border-t pt-4">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <button type="button" @click="addComponent()"
                            class="text-xs bg-blue-50 hover:bg-blue-100 text-blue-700 font-medium px-3 py-1.5 rounded-lg transition-colors">
                        + Add Component
                    </button>
                </div>
                    ...
                    <template x-for="(comp, index) in components" :key="index">
                        <div class="grid grid-cols-12 gap-2 items-center bg-gray-50 p-3 rounded-lg border">
                            <div class="col-span-4">
                                <input type="text" :name="`components[${index}][name]`" x-bind:required="!hasSubSubjects"
                                       x-model="comp.name" placeholder="Component (e.g. MCQ)"
                                       class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:ring-1 focus:ring-blue-400 outline-none">
                            </div>
                            <div class="col-span-3">
                                <input type="number" :name="`components[${index}][full]`" x-bind:required="!hasSubSubjects"
                                       x-model="comp.full" placeholder="Full Marks" min="1" step="0.01"
                                       class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:ring-1 focus:ring-blue-400 outline-none">
                            </div>
                            <div class="col-span-3">
                                <input type="number" :name="`components[${index}][pass]`" x-bind:required="!hasSubSubjects"
                                       x-model="comp.pass" placeholder="Pass Marks" min="1" step="0.01"
                                       class="w-full border border-gray-300 rounded px-2 py-1.5 text-sm focus:ring-1 focus:ring-blue-400 outline-none">
                            </div>
                            <div class="col-span-2 text-right">
                                <button type="button" @click="removeComponent(index)" x-show="components.length > 1"
                                        class="text-red-500 hover:text-red-700 text-xs font-medium px-2 py-1 rounded hover:bg-red-50">✕</button>
                            </div>
                        </div>
                    </template>
                </div>
...
function subjectForm() {
    return {
        hasSubSubjects: <?php echo e(old('has_sub_subjects') ? 'true' : 'false'); ?>,
        components: [{ name: 'MCQ', full: 40, pass: 16 }, { name: 'CQ', full: 60, pass: 24 }],
        subSubjects: [
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/tahmids55/Deployment/markscraft/resources/views/subjects/create.blade.php ENDPATH**/ ?>